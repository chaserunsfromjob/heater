...
- Land a document-only change — every changed path `.md` or `.txt`, or under `handover/` or `research/results/` — on one round passing with wording-only findings, which the stoker applies itself and records why in the store note (`bin/dispatch.py land`).
