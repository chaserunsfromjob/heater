...
- [x] **7. Worktree slots**, leased on demand rather than provisioned. The first
      ...
      deleted until its commits are provably in the trunk. A checkout whose
      tool calls are still returning is never removed by the sweep, whatever
      state its branch is in: silence for half an hour (`STALE_MINUTES`) is
      what says a worker has gone. The day-old rule
      (`EMPTY_BRANCH_STALE_HOURS`) applies only to a checkout with nothing
      committed on it, which git cannot tell from work that already landed.
