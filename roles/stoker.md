...
- Stop the lowest-priority dispatches still running with `bin/dispatch.py close <id> --outcome abandoned` once the band tightens, and spend the window that is left on the top of the list.
- Stop every running agent with `bin/dispatch.py close <id> --outcome abandoned` where the band reads NOTHING_NEW, then send the operator what `bin/debrief.py --hours 5 --queue` writes.
